...
def makeblastdb(
        ...
        type: Literal["nucl"] | Literal["prot"]
):
    """Run `makeblastdb` to create a blast database for the given FASTA file.
    Specify `type="nucl"` for nucleotide FASTA files and `type="prot"` for
    amino acids."""
...
def _blastnp(
        executable: Literal["blastn"] | Literal["blastp"],
        query_filename: pathlib.Path,
        db_name: pathlib.Path
):
    query_filename = query_filename.absolute()

    # We use -outfmt 13, which creates a json file per query sequence entry.
    # These files are being written to this temporary directory by
    # blastn/blastp, read by us and then the directory is deleted again.
    with tempfile.TemporaryDirectory() as directory:
        ...
        for filename in pathlib.Path(directory).glob("out*.json"):
            with open(filename, "r") as f:
                # All relevant results are under BlastOutput2 - report -
                # results - search
                entry = json.load(
                    f)['BlastOutput2']["report"]["results"]["search"]
            # Gather all hits, and all hsps for each hit, saving the target
            # title.
            hits = [(hit['description'][0]['title'].split(' ')[0].strip(), hsp)
                    for hit in entry["hits"] for hsp in hit["hsps"]]
            if hits:
    ...
    # Return a DataFrame with all the data for the lowest-evalue hit for each
    # query sequence.
...
class FASTAFile(abc.Mapping):
    ...
    def __init__(self, filename: pathlib.Path, type="nucl"):
        """Instantiate a FASTAFile with the given file name and type
        (either `nucl` or `prot`)."""
    ...
    def __getitem__(self, key: abc.Sequence[str] | str):
        ...
        if (
            ...
            (
                isinstance(key, abc.Sequence) or
    ...
    def query(self, other: FASTAFile):
        """Run blastn or blastp with this FASTA file as the reference and the
        given FASTA file as the query."""
        self._index()
        if self.type == "nucl":
    ...
    def translate(self):
        """Translate the nucleotide sequences in this file to amino acid
        sequences and return them as a new temporary FASTA file object."""
        if self.type != "nucl":
            raise ValueError(
                "Cannot translate FASTA files with a type other than 'nucl'.")
        protein_sequences = {
            name: Seq(_pad_Ns(sequence)).translate()
    ...
    def from_dict(
        ...
        filename: None | pathlib.Path = None,
        type="nucl"
    ):
...
class TemporaryFASTAFile(FASTAFile):
    ...
    def __init__(self, sequences: dict[str, str], type="nucl"):
        """Instantiate a temporary FASTA file, saving the given sequences to a
        temporary known location. This is deleted during object destruction."""
...
def reciprocal_best_blast(a: FASTAFile, b: FASTAFile):
    """Identify the reciprocal best blast hits between the two given FASTA
    files."""
    ...
    # Get the reverse best blast hit from the identified sequences in a
    # (...and back again).
    df_back = a.query(reciprocal_query)
    # Join the two DataFrames on the "b" column (since the query names are
    # from "b" in the back step)
    df: pd.DataFrame = df_there.join(
        df_back, on="b", how="inner", lsuffix="there", rsuffix="back")
    # Only retain entries in which the back step identified the same entry as
    # we started with in the there step.
    return df[df.index == df.bback].reset_index()


def _insertion(alignedpseq: str, trimmednuclseq: str) -> str:
    # Take the given aligned amino acid sequence and insert --- for each
    # inserted amino acid.
    finalnuclseq: str = ""
...
def _calculate_Ka(r: pd.Series) -> int:
    # Count the number of "+" and " " in the blast midline from the back step,
    # this gives us the number of non-synonymous mutations.
    midline: str = r.midlineback
    return midline.replace("+", " ").count(" ")


def _calculate_KsKa(r: pd.Series):
    # Correct the nucleotide sequences with any insertions from the alignment
    # of the amino acid sequences
    # (i.e. insert "---" into the nucl sequences every time we encounter a "-"
    # in the prot sequences).
    a = _insertion(r.hseqback, r.anuclseq[(
        r.hit_fromback-1)*3:(r.hit_toback-1)*3+3])
    b = _insertion(r.qseqback, r.bnuclseq[(
        r.query_fromback-1)*3:(r.query_toback-1)*3+3])
...
def _row_KaKs(r: pd.Series):
    try:
        ...
        Ka = _calculate_Ka(r)
        # Get the number of synonymous mutations by identifying all kmer
        # changes and then subtracting Ka
...
def kaks_multiple(
        query: FASTAFile,
        references: list[FASTAFile]
) -> pd.DataFrame:
    """Calculate Ka/Ks of the query sequences against each of the given
...
def cli_main():
    parser = argparse.ArgumentParser(
        description=(
            "Perform a Ka/Ks analysis for the given nucleotide query "
            "and reference FASTA files. The result is given in CSV format to "
            "stdout."
        )
    )
    parser.add_argument("query", type=FASTAFile,
                        help="The filename of the query FASTA.")
    parser.add_argument("reference", nargs="+", type=FASTAFile,
                        help="The filename(s) of the reference FASTA.")
...
def cli_rbb():
    parser = argparse.ArgumentParser(
        description=(
            "Perform a reciprocal best BLAST analysis for the given "
            "query and reference FASTA files. The result is given in CSV "
            "format to stdout."
        )
    )
    ...
    parser.add_argument("reference", type=pathlib.Path,
                        help="The filename(s) of the reference FASTA.")
    parser.add_argument("--query-type", choices=["nucl", "prot"],
                        default="nucl",
                        help=(
                            "The type of query, nucleotide ('nucl') or "
                            "protein ('prot'). Default is 'prot'."))
